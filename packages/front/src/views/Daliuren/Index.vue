  ...
  if (container.value) {
    ...
    pan.handleClick(() => {
      nextTick(() => {
        ...
        const keInfo = qiKe(jiashi, monthIndex.value, guiRenOnBranch.value).useList()
        answerBase.value = keInfo
        pan.updateActiveLabel(usePanLabel(keInfo, dateTimeInfo.value.d))
      })
    });
  }
});
